...
const consoleMethods = {
  ...
  clear() {
    ...
    if (this._stdout.isTTY && process.env.TERM !== "dumb") {
      ...
      const { cursorTo, clearScreenDown } =
        require("../readline/callbacks").default;
  ...
  group(...data) {
    ...
    this[kGroupIndent] += String.prototype.repeat.call(
      " ",
  ...
  groupEnd() {
    this[kGroupIndent] = String.prototype.slice.call(
      this[kGroupIndent],
      0,
  ...
  table(tabularData, properties) {
    ...
    if (cliTable === undefined) cliTable = require("../cli_table").default;
...
function timeLogImpl(self, name, label, data) {
  ...
  if (data === undefined) {
    ...
    self.log("%s: %s", label, formatted, ...data[Symbol.iterator]());
  }
  return true;
}
...
function formatTime(ms) {
  ...
  if (hours !== 0 || minutes !== 0) {
    ({ 0: seconds, 1: ms } = String.prototype.split.call(
      Number.prototype.toFixed(seconds, 3),
      "."
    ));
  ...
  if (seconds !== 0) {
    return `${Number.prototype.toFixed(seconds, 3)}s`;
  }

  return `${Number(Number.prototype.toFixed(ms, 3))}ms`;
}
...
const isArray = (v) => v instanceof Array || isTypedArray(v) || isBuffer(v);

...
// } = internalBinding('util');
  const getOwnNonIndexProperties = false;
  const getPromiseDetails = false;
  const getProxyDetails = false;
  const kPending = false;
  const kRejected = false;
  const previewEntries = false;
  const internalGetConstructorName = false;
  const getExternalValue = false;
  const ALL_PROPERTIES = false;
  const ONLY_ENUMERABLE = false

import { customInspectSymbol, isError, join, removeColors } from "../util.js";
  ...
  if (value[SymbolIterator] || constructor === null) {
    ...
    if (ArrayIsArray(value)) {
      ...
      if (constructor === null) {
        ...
        bound = new require("@darkwolf/primordials")[fallback](value);
      }

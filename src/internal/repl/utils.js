...
const { ERR_INSPECTOR_NOT_AVAILABLE } = codes;
      ...
      raise(pos, message) {
        switch (message) {
          ...
          case "Unterminated string constant":
            const token = String.prototype.slice.call(
              this.input,
              this.lastTokStart,
              this.pos
            );
...
function setupPreview(repl, contextSymbol, bufferSymbol, active) {
  ...
  function showCompletionPreview(line, insertPreview) {
    ...
    repl.completer(line, (error, data) => {
      ...
      const completions = Array.prototype.filter.call(rawCompletions, Boolean);
      ...
      const suffix = String.prototype.slice.call(prefix, completeOn.length);
  ...
  function isInStrictMode(repl) {
    return (
      ...
      Array.prototype.includes.call(
        Array.prototype.map.call(process.execArgv, (e) =>
          String.prototype.replace.call(
            String.prototype.toLowerCase.call(e),
            /_/g,
            "-"
          )
        ),
  ...
  function getInputPreview(input, callback) {
    ...
    if (
      String.prototype.startsWith.call(input, "{") &&
      !String.prototype.endsWith.call(input, ";") &&
      !wrapped
    ) {
      input = `(${input})`;
      wrapped = true;
  ...
  const showPreview = () => {
    ...
    const line = String.prototype.trim.call(repl.line);
    ...
    const inputPreviewCallback = (error, inspected) => {
      ...
      if (
        ...
        let res = "";
        for (const char of inspected[Symbol.iterator]()) {
      ...
      const lineBreakPos = String.prototype.indexOf.call(inspected, "\n");
      if (lineBreakPos !== -1) {
        inspected = `${String.prototype.slice.call(
          inspected,
          0,
          lineBreakPos
        )}`;
...
function setupReverseSearch(repl) {
  ...
  function search() {
    ...
    while (historyIndex >= 0 && historyIndex < repl.history.length) {
      ...
      if (dir === "r") {
        ...
        cursor = String.prototype.lastIndexOf.call(entry, input, cursor - 1);
      } else {
        cursor = String.prototype.indexOf.call(entry, input, cursor + 1);
      }
      // Match not found.
      if (cursor === -1) {
        ...
        if (repl.useColors) {
          const start = String.prototype.slice.call(entry, 0, cursor);
          const end = String.prototype.slice.call(entry, cursor + input.length);
  ...
  function print(outputLine, inputLine, cursor = repl.cursor) {
    ...
    let rows = 0;
    if (lastMatch !== -1) {
      const line = String.prototype.slice.call(
        repl.history[lastMatch],
        0,
        lastCursor
      );
    ...
    const cursorLine =
      prompt + String.prototype.slice.call(outputLine, 0, cursor);
  ...
  function reverseSearch(string, key) {
    if (!isInReverseSearch) {
      ...
      reset(String.prototype.slice.call(input, 0, input.length - 1));
      search();
